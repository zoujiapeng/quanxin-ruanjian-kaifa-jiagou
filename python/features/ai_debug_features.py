...
@feature(
    ...
    description="【AI 规划】将自然语言任务描述转换为 Lobster DSL。用于自然语言到自动化流程的转换，生成的 DSL 可由 run_dsl 执行",
    category=F.AI,
    params=[
...
@feature(
    ...
    description="【可视化】将 DSL 文本转换为 React Flow 格式的节点/边 JSON。用于展示 DSL 流程图或在 UI 中编辑",
    category=F.AI,
    params=[
...
@feature(
    ...
    description="【可视化·反向】将 React Flow 节点/边 JSON 转换回 DSL 文本。在图编辑器中编辑流程后转回 DSL 执行",
    category=F.AI,
    params=[
...
@feature(
    name="debug_get_logs",
    display_name="获取执行日志",
    description="【调试·日志】返回执行器最近的日志。用于诊断 DSL 执行失败原因、查看执行历史",
    category=F.DEBUG,
    params=[
...
@feature(
    ...
    display_name="运行测试",
    description="【调试·测试】运行所有注册功能的测试用例。用于系统升级后验证完整性和回归测试",
    category=F.DEBUG,
    params=[
...
@feature(
    ...
    display_name="列出功能",
    description="【调试·发现】返回所有注册功能的完整信息。format=markdown 返回 Claude 可读格式，format=mcp 返回 MCP schema。Claude Code 发现 Lobster 能力的入口",
    category=F.DEBUG,
    params=[
...
@feature(
    ...
    description="【调试·DSL验证】解析并验证 DSL 文本语法，返回 AST 和错误信息。推荐执行 DSL 前先验证",
    category=F.DEBUG,
    params=[
        P("dsl", "str", "DSL文本", example="CLICK 开始\nWAIT 完成"),
    ],
...
@feature(
    ...
    display_name="系统诊断",
    description="【调试·诊断】返回系统状态、依赖检查、功能统计。用于排查环境问题时快速诊断",
    category=F.DEBUG,
    params=[],
    returns="dict - 系统诊断报告",
